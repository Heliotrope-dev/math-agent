...
收到知识讲解请求时，禁止直接解算具体题目，禁止输出"解："或"答案："。
必须按照以下四步结构输出，不得跳步：
...
如果用表格形式呈现"错误做法/正确做法/说明"，表格单元格里的公式必须
保持简单、独立、写完整——不要用 \\boldsymbol{}、\\underbrace{} 这类会跨多个
字符、容易在单元格里断开的复杂嵌套命令，也不要让一个公式被表格的 | 分隔符
截断到另一个单元格里。宁可用简单一点的写法（比如直接写 $z-z_0$ 而不是
$\\boldsymbol{z-z_0}$），也不要让公式写到一半断掉、内容错位飘到别的格子里。
...
效率要求：如果同一步骤里需要算多个互不依赖的数值（比如比较区间端点和
多个临界点的函数值、验证多组解），把这些 calculator 调用打包在同一轮里
一次性发起，不要为每个数值单独发起一轮工具调用——这样能明显减少往返轮数、
加快响应，前提是这些计算之间确实没有先后依赖关系。

用户说"讲解一下"时，结合上下文解释这道题的完整解题过程和思路，不重复题目。

诚实原则（硬性要求，优先级高于"给出完整解答"）：
...
- 禁止在标题、正文里加装饰性 emoji（比如在小标题前加🧭📌✨这类图标）——
  这是一个数学工具，要课本一样干净严肃的排版，不要"AI助手常见"的活泼风格。
  数学符号本身（√ ∞ π ≤ 这类）不受此限制，正常使用。
- 禁止把内部自我修正过程写进正文（比如"工具返回的结果与预期不符，我重新核对一遍"
  这类话）——发现计算异常就直接重算、直接用修正后的结果作答，不要向学生描述
  你刚才纠错的过程，正文只呈现面向学生的干净讲解。"""
...
- 回复中文
- 不加装饰性 emoji，课本风格排版
- 禁止把内部自我修正过程写进正文（比如"工具返回的结果与预期不符，我重新核对
  一遍"这类话）——发现异常就直接重算后直接用结果引导，不要向学生描述这个过程
...
class MathAgent:
    ...
    def solve(
        ...
        if image_bytes:
            if not self.supports_vision:
                return "当前模型不支持图片识别，请使用拍题功能或切换到视觉模型。"
    ...
    def _solve_stream_with_tools(
        ...
        try:
            ...
            for c in _fake_stream("解题超时，请尝试把题目拆分成更小的步骤发送。"):
                yield c


def _iter_with_timeout(iterable, timeout_seconds: float = 45.0):
    """给任意（可能会卡住不动的）迭代器包一层"看门狗"超时。

    httpx client 上配置的 timeout=60s 理论上该覆盖流式读取，但实测线上真的
    卡死过一次（连接没报错也没断开，就是不再产生新数据），说明底层超时在
    流式场景下不完全可靠。这里用一个后台线程真正消费底层迭代器、通过队列
    转发给调用方；如果连续 timeout_seconds 秒队列里都没有新东西，直接抛
    TimeoutError，不再无限期等待。

    注意：如果底层迭代器真的永久卡死，那个后台线程会一直挂在那儓出不来
    （Python 没法强制杀死线程），但它是 daemon 线程，不会阻止主流程继续、
    不会阻止用户看到超时提示——只是会有一次性的线程泄漏，可以接受，好过
    让用户对着卡死的页面干等。
    """
    ...
    def _worker():
        try:
            for item in iterable:
                q.put(item)
            q.put(_SENTINEL)
        except Exception as exc:  # noqa: BLE001 — 转发给消费方，不在这里吞掉
            q.put(exc)
    ...
    while True:
        try:
            ...
            raise TimeoutError(f"流式响应卡住：连续 {timeout_seconds} 秒没有收到新数据")
