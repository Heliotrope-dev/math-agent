...
# ── route_model ───────────────────────────────────────────────────────────────
# gemini-3.5-flash-lite 原生多模态，文字/图片统一路由到同一个模型，
# 不再有单独的视觉供应商可切换。

def test_route_model_no_image_returns_gemini():
    assert route_model("求导数", image_bytes=None) == "gemini-3.5-flash-lite"


def test_route_model_with_image_also_returns_gemini():
    assert route_model("拍题", image_bytes=b"fake") == "gemini-3.5-flash-lite"
