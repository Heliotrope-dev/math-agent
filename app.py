...
""")
    ...
    with st.expander("🎓 课程入口", expanded=True):
        ...
        for _i, _course in enumerate(["数学分析", "高等代数", "解析几何"]):
            with (_c1 if _i % 2 == 0 else _c2):
                if st.button(_course, key=f"course_{_i}", use_container_width=True):
                    st.session_state["prefill"] = f"我想系统复习{_course}，请先给我一个完整的知识框架，然后从最核心的概念开始讲解。"
                    st.rerun()
        st.caption("大二")
        _c3, _c4 = st.columns(2)
        for _i, _course in enumerate(["常微分方程", "复变函数", "抽象代数", "概率论"]):
            with (_c3 if _i % 2 == 0 else _c4):
                if st.button(_course, key=f"course_{_i+3}", use_container_width=True):
                    st.session_state["prefill"] = f"我想系统复习{_course}，请先给我一个完整的知识框架，然后从最核心的概念开始讲解。"
                    st.rerun()
        st.caption("大三")
        _c5, _c6 = st.columns(2)
        for _i, _course in enumerate(["实变函数", "泛函分析", "点集拓扑", "微分几何", "数值分析", "偏微分方程", "数学建模"]):
            with (_c5 if _i % 2 == 0 else _c6):
                if st.button(_course, key=f"course_{_i+7}", use_container_width=True):
                    st.session_state["prefill"] = f"我想系统复习{_course}，请先给我一个完整的知识框架，然后从最核心的概念开始讲解。"
                    st.rerun()
