...
- **对话持久化**：历史消息 + 图片 + 思维导图存 Supabase，刷新页面不丢失；老用户再次打开显示"欢迎回来"个性化提示（上次学到哪、待复习错题数），新用户仍是示例题引导
...
`RAGEngine` 被 `@st.cache_resource` 缓存成进程内唯一实例，连着唯一一个 ChromaDB
collection——从文档切分（`chunk_id`）到检索（`query`）到列表/删除，全程没有任何
用户维度的过滤。这个问题是交给 Fable 5 做独立代码审查时揪出来的，不是自己先发现
的。实际后果：两个真实账号同时用这个功能，A 提问会检索到 B 的私有文档内容并被
AI 引用作答，侧边栏"已上传文档"列表是全局共享的（A 能看到 B 传过什么文件名），
甚至能直接点删除把 B 的文档删掉，两人传同名文件还会互相覆盖。这比上面"已知的安全
权衡"里提到的 RLS 问题更严重——RLS 好歹有应用层邮箱过滤兜底，只在 anon key 泄露
时才会被利用；这个是**只要两个账号正常使用，必然触发**，不需要任何额外条件。

修复思路：`chunk_id` 和 metadata 都加上 `user` 字段，`add_documents`/`query`/
`list_documents`/`delete_document` 全部按 `where={"user": email}` 过滤；同名
文件的"先删后写"去重逻辑也收窄成只删同一用户名下的旧版本，不再影响其他用户。
用真实账号在 VPS 上模拟两个用户完整跑过一遍添加/检索/删除，确认三条路径都不
再互相可见。**教训：`@st.cache_resource`/全局单例这类为了"复用连接、省初始化
开销"而设计的缓存机制，很容易在无意中把"进程级共享"和"本该按用户隔离"的数据
边界混为一谈——加新的持久化存储时，"要不要按用户过滤"应该是默认要问的第一个
问题，而不是等多用户场景出问题才想起来查。**

**知识库"参考来源"渲染是存储型 XSS，配合登录 token 能账号接管**

`pages/2_知识库问答.py` 展开"参考来源"时，检索到的原文片段直接 `unsafe_allow_html=True` 渲染，没有转义。而登录 token 明文存在 `localStorage`——攻击者往一份"学习资料"里塞一段 `<img onerror=...>`，分享给别人，对方上传到自己的知识库后点开来源展开就中招，token 被读走等于账号被接管。这个问题是交给 Fable 5 做独立代码审查揪出来的（跟之前 RAG 隔离 bug 是同一个套路）。同一批审查还带出另外几个没对齐的地方：`plot_function` 生成图片的标题走的是跟 `draw_mindmap` 不一样的未转义渲染路径；每日调用配额限流漏了两条口子（存错题本时的摘要生成、扫描件 PDF 的逐页 OCR），反复点相关按钮能绕开限流白嫖付费 API；"学习档案"的访问时间戳（`_track_topic`）还留着一处裸 `datetime.now()`，是前面"登录锁定的时区裸比较"那个坑的第二次复发；使用手册写的"展开工具调用详情"功能实际上代码里从没渲染过收集到的 trace 数据，是纯粹的死代码。全部已修复。**教训：同一类问题（未转义渲染、时区裸比较）即使在别处已经修过，新增的类似路径也不会自动跟着对齐，得靠独立审查或者抽成公共函数强制复用才能真正堵住。**

**AI 回复偶尔混入"我重新核对一遍"这类内部独白**

解题/引导过程中，模型偶尔会把工具调用后的自我修正过程（"工具返回的结果与预期不符，我重新核对一遍，确保例题数据准确无误"）直接写进给学生看的正文，读起来跟课本风格的其它内容很违和。这是真实用户测试（同样交给 Fable 5 做，这次是真的把应用跑起来当用户操作了一遍，不只是读代码）中发现的，属于模型行为层面的问题，不是代码 bug。修复：两个系统提示词（解题/引导模式）都加了一条格式规则，明确要求"发现异常直接重算，不要向学生描述这个纠错过程"。

**GitHub Actions 自动部署的密钥在服务器迁移后没跟着换，静默失效了一个多月**

8/13-14 那次把主力服务器搬到新机器（179.255.101.244）之后，GitHub 仓库里 `VPS_HOST`/`VPS_SSH_KEY`/`VPS_USER` 这三个 Secrets 还留着 7/3 那次设的老值，指向已经废弃的旧服务器/旧密钥。部署失败又没有任何提醒，一直没人发现，直到下一次 push 才在 Actions 页面看到红叉——也就是说这期间推到 main 的改动其实都没真正上线，只是以为部署了。改密钥时还踩了一次网页表单的坑：私钥这种多行文本直接粘贴进浏览器 Secret 输入框，格式很容易悄悄被截断或丢换行，"看起来存上了"但 SSH 握手其实一直失败——换成 `gh secret set VPS_SSH_KEY < 私钥文件` 从文件直接写才排除。**教训：部署密钥、服务器地址这类基础设施配置，迁移服务器时容易只顾着搬代码本身，忘了同步检查；多行密文这种东西尽量用文件重定向写，别指望网页表单粘贴靠谱。**

**公式检索的 RAG 模式静默退化成关键词匹配，谁都没发现**

`formula_lookup` 走 RAG（`rag_formula_lookup.py`，语义检索、支持自然语言查询）还是关键词字典 fallback，取决于 `_get_rag_index()` 能不能连上 Ollama——连不上会直接吞掉异常降级，界面上完全看不出区别，用户只会觉得"检索到的公式好像不太准"。排查下来发现两层问题：一是生产环境 `OLLAMA_BASE_URL` 配的是服务器自己的公网 IP（`179.255.101.244:11434`），但这个端口实际是 frp 从我本机 Ollama 反向隧道过来的——Ollama 自带的 Host 头校验只认 `127.0.0.1`/`localhost`，用公网 IP 当 Host 直接被拒 403（服务器请求自己的公网 IP 都一样会被拒，隧道原样透传 Host 头）；二是本机 Ollama 那时候压根没装 `nomic-embed-text` 这个 embedding 模型，装了也白搭。两个问题叠在一起，这个功能大概率从服务器迁移那次开始就一直在用 fallback，没人注意到。修复：`OLLAMA_BASE_URL` 改成 `127.0.0.1:11434`（同一台机器上，走本地 Host 头能过校验），本机补拉 `nomic-embed-text`，改完直接在 VPS 上跑了一遍 `build_index()`+`retrieve()` 确认真的命中 RAG 检索而不是猜的。**教训：这类"连不上就默默降级"的设计对用户体验友好，但也意味着功能实际失效了也没人会报错，得靠主动去验证输出内容才能发现，光看"页面没崩"不代表功能是好的。**
